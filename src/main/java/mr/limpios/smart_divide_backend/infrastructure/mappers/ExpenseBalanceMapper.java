...
public class ExpenseBalanceMapper {
  ...
  public static ExpenseBalanceSchema toSchema(ExpenseBalance balance, ExpenseSchema expenseSchema) {
  ...
  public static ExpenseBalance toModel(ExpenseBalanceSchema balanceSchema) {
    return new ExpenseBalance(balanceSchema.getId(),
        UserMapper.toModel(balanceSchema.getCreditor()),
        UserMapper.toModel(balanceSchema.getDebtor()), balanceSchema.getAmountToPaid());
  }

  public static List<ExpenseBalanceSchema> toSchemaList(List<ExpenseBalance> balances,
      ExpenseSchema expenseSchema) {
    return balances.stream().map(balance -> toSchema(balance, expenseSchema))
        .collect(Collectors.toList());
  }

  public static List<ExpenseBalance> toModelList(List<ExpenseBalanceSchema> balanceSchemas) {
    return balanceSchemas.stream().map(ExpenseBalanceMapper::toModel).collect(Collectors.toList());
  }
}
