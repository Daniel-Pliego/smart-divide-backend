...
public class ExpenseParticipantMapper {
  ...
  public static ExpenseParticipantSchema toSchema(ExpenseParticipant participant,
  ...
  public static ExpenseParticipant toModel(ExpenseParticipantSchema participantSchema) {
    return new ExpenseParticipant(participantSchema.getId(),
        UserMapper.toModel(participantSchema.getPayer()), participantSchema.getAmountPaid(),
        participantSchema.getMustPaid());
  }

  public static List<ExpenseParticipantSchema> toSchemaList(List<ExpenseParticipant> participants,
      ExpenseSchema expenseSchema) {
    return participants.stream().map(participant -> toSchema(participant, expenseSchema))
        .collect(Collectors.toList());
  }

  public static List<ExpenseParticipant> toModelList(
      List<ExpenseParticipantSchema> participantSchemas) {
    return participantSchemas.stream().map(ExpenseParticipantMapper::toModel)
        .collect(Collectors.toList());
  }
