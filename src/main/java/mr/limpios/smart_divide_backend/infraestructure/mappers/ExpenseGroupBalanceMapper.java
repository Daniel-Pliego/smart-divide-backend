...
public class ExpenseGroupBalanceMapper {
    ...
    public static ExpenseGroupBalanceSchema toSchema(ExpenseGroupBalance expenseGB) {
        return new ExpenseGroupBalanceSchema(
                expenseGB.id(),
                UserMapper.toSchema(expenseGB.creditor()),
                UserMapper.toSchema(expenseGB.debtor()),
                expenseGB.amount(),
                GroupMapper.toSchema(expenseGB.group()));
    }

    public static ExpenseGroupBalance toModel(ExpenseGroupBalanceSchema expenseGBS){
        return new ExpenseGroupBalance(
            expenseGBS.getId(),
            UserMapper.toModel(expenseGBS.getCreditor()),
            UserMapper.toModel(expenseGBS.getDebtor()),
            expenseGBS.getAmount(),
