...
public class PaymentMapper {
    ...
    public static Payment toModel(PaymentSchema paymentSchema){
        return new Payment(
                paymentSchema.getId(),
                UserMapper.toModel(paymentSchema.getFromUser()),
                UserMapper.toModel(paymentSchema.getToUser()),
                paymentSchema.getAmount(),
                GroupMapper.toModel(paymentSchema.getGroup()),
    ...
    public static PaymentSchema toSchema(Payment payment) {
        return PaymentSchema.builder()
                ...
                .toUser(UserMapper.toSchema(payment.toUser()))
                .amount(payment.amount())
                .group(GroupMapper.toSchema(payment.group()))
